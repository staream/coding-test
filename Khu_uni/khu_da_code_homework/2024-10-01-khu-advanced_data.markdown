...
다중비교 검정은 최초에 분산분석을 실행하고, 도출한 p값이 알파보다 작을 떄 사용하는 것이 일반적이다.

다중비교에는 F분포를 이용한 검정방법인 분산분석과 마찬가지, 아닌것
1) 마찬가지 : 통계적으로 유의미하다/안하다
2) 후자 : 분산분석에는 유의미하지 않았지만 다중비교에서는 유의미한 쌍이 있는 등, 결과가 다름
...
### 카이제곱검정 : 독립성 검정
이전의 것은 데이터 vs 모집단의 확률분포

독립은 범주가 달라져도 비율이 달라지지 않는 것
...
피어슨 상관계수- 양적 변수 간 관계의 강도를 정량화하는 바엉
- 선형에 얼마나 가까운가?
